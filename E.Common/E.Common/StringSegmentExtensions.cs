...
namespace E.Common
{
    ...
    {
        private const string BAD_FORMAT = "Input string was not in a correct format.";
        private const string OVERFLOW_MESSAGE = "Value was either too large or too small for an {0}.";
        ...
        public static bool ParseBoolean(this StringSegment value)
        {
            if (!value.TryParseBoolean(out bool result))
                throw new FormatException(BAD_FORMAT);
        ...
        private static Exception CreateOverflowException(long maxValue) =>
            new OverflowException(string.Format(OVERFLOW_MESSAGE, SignedMaxValueToIntType(maxValue)));
        private static Exception CreateOverflowException(ulong maxValue) =>
            new OverflowException(string.Format(OVERFLOW_MESSAGE, UnsignedMaxValueToIntType(maxValue)));
            ...
            while (i < end)
            {
                ...
                switch (state)
                {
                    case ParseState.LeadingWhite:
                        ...
                        else throw new FormatException(BAD_FORMAT);
                        break;
