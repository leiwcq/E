...
namespace E.Common
{
    ...
    public static class ReflectionExtensions
    {
        ...
        static Dictionary<Type, EmptyCtorDelegate> _constructorMethods = new Dictionary<Type, EmptyCtorDelegate>();
        public static EmptyCtorDelegate GetConstructorMethod(Type type)
        {
            if (_constructorMethods.TryGetValue(type, out var emptyCtorFn)) return emptyCtorFn;
            ...
            do
            {
                ...
                newCache = new Dictionary<Type, EmptyCtorDelegate>(_constructorMethods)
                {
                    [type] = emptyCtorFn
                };

            } while (!ReferenceEquals(
                Interlocked.CompareExchange(ref _constructorMethods, newCache, snapshot), snapshot));

            return emptyCtorFn;
        }

        static Dictionary<string, EmptyCtorDelegate> _typeNamesMap = new Dictionary<string, EmptyCtorDelegate>();
        public static EmptyCtorDelegate GetConstructorMethod(string typeName)
        {
            if (_typeNamesMap.TryGetValue(typeName, out var emptyCtorFn)) return emptyCtorFn;
            ...
            do
            {
                snapshot = _typeNamesMap;
                newCache = new Dictionary<string, EmptyCtorDelegate>(_typeNamesMap) {
                    [typeName] = emptyCtorFn
                };

            } while (!ReferenceEquals(
                Interlocked.CompareExchange(ref _typeNamesMap, newCache, snapshot), snapshot));
        ...
        {
            var factoryFn = GetConstructorMethod(type);
            return (T)factoryFn();
        }
        ...
        public static string[] IgnoreAttributesNamed = new[] {
        ...
        public static DataContractAttribute GetDataContract(this Type type)
        {
            ...
            if (dataContract == null && Common.Env.IsMono)
        ...
        public static DataMemberAttribute GetDataMember(this PropertyInfo pi)
        {
            ...
            if (dataMember == null && Common.Env.IsMono)
