...
namespace E.Common.Support
{
    public class TimeSpanConverter
    {
        ...
        public static TimeSpan FromXsdDuration(string xsdDuration)
        {
            ...
            if (d.Length == 2)
            {
                if (int.TryParse(d[0], out int day))
                    days = day;
            }
            if (hasTime)
            {
                ...
                if (h.Length == 2)
                {
                    if (int.TryParse(h[0], out int hour))
                        hours = hour;
                }
                ...
                if (m.Length == 2)
                {
                    if (int.TryParse(m[0], out int min))
                        minutes = min;
                }
                ...
                if (s.Length == 2)
                {
                    if (decimal.TryParse(s[0], NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out decimal millis))
                        seconds = millis;
                }
            }
