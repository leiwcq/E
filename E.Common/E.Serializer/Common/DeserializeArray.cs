...
namespace E.Serializer.Common
{
    ...
    {
        private static Dictionary<Type, ParseArrayOfElementsDelegate> _parseDelegateCache
        ...
        {
            if (_parseDelegateCache.TryGetValue(type, out var parseFn)) return parseFn.Invoke;
            ...
            do
            {
                ...
                newCache = new Dictionary<Type, ParseArrayOfElementsDelegate>(_parseDelegateCache)
                {
                    [type] = parseFn
                };
    ...
    internal static class DeserializeArray<TSerializer>
        where TSerializer : ITypeSerializer
    {
        private static Dictionary<Type, ParseStringSegmentDelegate> _parseDelegateCache = new Dictionary<Type, ParseStringSegmentDelegate>();
        ...
        public static ParseStringSegmentDelegate GetParseStringSegmentFn(Type type)
        {
            ...
            if (_parseDelegateCache.TryGetValue(type, out parseFn)) return parseFn;
            ...
            {
                snapshot = _parseDelegateCache;
                newCache = new Dictionary<Type, ParseStringSegmentDelegate>(_parseDelegateCache);
