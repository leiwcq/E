...
namespace E.Serializer.Common
{
    ...
    {
        static Dictionary<Type, TypeFields> _cacheMap = new Dictionary<Type, TypeFields>();

        public static Type FactoryType = typeof(TypeFields<>);

        public static TypeFields Get(Type type)
        {
            if (_cacheMap.TryGetValue(type, out TypeFields value))
                return value;
            ...
            {
                snapshot = _cacheMap;
                newCache = new Dictionary<Type, TypeFields>(_cacheMap)
                {
                    [type] = instance
                };
            } while (!ReferenceEquals(
                Interlocked.CompareExchange(ref _cacheMap, newCache, snapshot), snapshot));
        ...
        public readonly Dictionary<string, FieldAccessor> FieldsMap =
            new Dictionary<string, FieldAccessor>(E.Common.PclExport.Instance.InvariantComparerIgnoreCase);
