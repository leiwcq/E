...
namespace E.Serializer.Common
{
        ...
        where TSerializer : ITypeSerializer
    {
        ...
        public static ParseStringSegmentDelegate GetParseStringSegmentMethod(Type type)
        {
            ...
            var keyTypeParseMethod = Serializer.GetParseStringSegmentFn(keyValuePairArgs[KEY_INDEX]);
            if (keyTypeParseMethod == null) return null;

            var valueTypeParseMethod = Serializer.GetParseStringSegmentFn(keyValuePairArgs[VALUE_INDEX]);
        ...
        private static Dictionary<string, ParseKeyValuePairDelegate> _parseDelegateCache
        ...
        private static readonly Type[] Signature = { typeof(StringSegment), typeof(Type), typeof(ParseStringSegmentDelegate), typeof(ParseStringSegmentDelegate) };
            ...
            ParseStringSegmentDelegate keyParseFn, ParseStringSegmentDelegate valueParseFn)
        {
            ...
            if (_parseDelegateCache.TryGetValue(key, out var parseDelegate))
            ...
            do
            {
                snapshot = _parseDelegateCache;
                newCache = new Dictionary<string, ParseKeyValuePairDelegate>(_parseDelegateCache)
                {
                    [key] = parseDelegate
                };

            } while (!ReferenceEquals(
                Interlocked.CompareExchange(ref _parseDelegateCache, newCache, snapshot), snapshot));
