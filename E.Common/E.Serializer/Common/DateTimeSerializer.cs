...
namespace E.Serializer.Common
{
    ...
    {
        public const string CONDENSED_DATE_TIME_FORMAT = "yyyyMMdd";                             //8
        public const string SHORT_DATE_TIME_FORMAT = "yyyy-MM-dd";                               //11
        public const string DEFAULT_DATE_TIME_FORMAT = "dd/MM/yyyy HH:mm:ss";                    //20
        public const string DEFAULT_DATE_TIME_FORMAT_WITH_FRACTION = "dd/MM/yyyy HH:mm:ss.fff";    //24
        public const string XSD_DATE_TIME_FORMAT = "yyyy-MM-ddTHH:mm:ss.fffffffZ";               //29
        public const string XSD_DATE_TIME_FORMAT3_F = "yyyy-MM-ddTHH:mm:ss.fffZ";                 //25
        public const string XSD_DATE_TIME_FORMAT_SECONDS = "yyyy-MM-ddTHH:mm:ssZ";                //21
        public const string DATE_TIME_FORMAT_SECONDS_UTC_OFFSET = "yyyy-MM-ddTHH:mm:sszzz";        //22
        public const string DATE_TIME_FORMAT_SECONDS_NO_OFFSET = "yyyy-MM-ddTHH:mm:ss";
        public const string DATE_TIME_FORMAT_TICKS_UTC_OFFSET = "yyyy-MM-ddTHH:mm:ss.fffffffzzz";  //30
        public const string DATE_TIME_FORMAT_TICKS_NO_UTC_OFFSET = "yyyy-MM-ddTHH:mm:ss.fffffff";

        public const string ESCAPED_WCF_JSON_PREFIX = "\\/Date(";
        public const string ESCAPED_WCF_JSON_SUFFIX = ")\\/";
        public const string WCF_JSON_PREFIX = "/Date(";
        public const char WCF_JSON_SUFFIX = ')';
        public const string UNSPECIFIED_OFFSET = "-0000";
        public const string UTC_OFFSET = "+0000";

        private const char XSD_TIME_SEPARATOR = 'T';
        private static readonly int XsdTimeSeparatorIndex = XSD_DATE_TIME_FORMAT.IndexOf(XSD_TIME_SEPARATOR);
        private const string XSD_UTC_SUFFIX = "Z";
        private static readonly char[] DateTimeSeperators = { '-', '/' };
        private static readonly Regex UtcOffsetInfoRegex = new Regex("([+-](?:2[0-3]|[0-1][0-9]):[0-5][0-9])", E.Common.PclExport.Instance.RegexOptions);
        ...
        public static DateTime ParseShortestXsdDateTime(string dateTimeStr)
        {
            try
            {
                ...
                if (dateTimeStr.StartsWith(ESCAPED_WCF_JSON_PREFIX, StringComparison.Ordinal) || dateTimeStr.StartsWith(WCF_JSON_PREFIX, StringComparison.Ordinal))
                    return ParseWcfJsonDate(dateTimeStr).Prepare();

                if (dateTimeStr.Length == DEFAULT_DATE_TIME_FORMAT.Length)
                {
                ...
                if (dateTimeStr.Length == DEFAULT_DATE_TIME_FORMAT_WITH_FRACTION.Length)
                {
                ...
                if (dateTimeStr.Length == XSD_DATE_TIME_FORMAT_SECONDS.Length)
                    return DateTime.ParseExact(dateTimeStr, XSD_DATE_TIME_FORMAT_SECONDS, null, DateTimeStyles.AdjustToUniversal).Prepare(parsedAsUtc: true);

                if (dateTimeStr.Length >= XSD_DATE_TIME_FORMAT3_F.Length
                    && dateTimeStr.Length <= XSD_DATE_TIME_FORMAT.Length
                    && dateTimeStr.EndsWith(XSD_UTC_SUFFIX))
                {
                ...
                if (dateTimeStr.Length == CONDENSED_DATE_TIME_FORMAT.Length && dateTimeStr.IndexOfAny(DateTimeSeperators) == -1)
                {
                    return DateTime.ParseExact(dateTimeStr, "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.None);
                }

                if (dateTimeStr.Length == SHORT_DATE_TIME_FORMAT.Length)
                {
                    try
                    {
        ...
        {
            if ((dateTimeStr.Length > XsdTimeSeparatorIndex) && (dateTimeStr[XsdTimeSeparatorIndex] == ' ') && dateTimeStr.EndsWith(XSD_UTC_SUFFIX))
            {
                dateTimeStr = dateTimeStr.Substring(0, XsdTimeSeparatorIndex) + XSD_TIME_SEPARATOR +
        ...
        {
            if (dateTimeStr == null || dateTimeStr.Length < SHORT_DATE_TIME_FORMAT.Length)
                return null;

            if (dateTimeStr.EndsWith(XSD_UTC_SUFFIX))
            {
        ...
        {
            return dateTime.ToStableUniversalTime().ToString(XSD_DATE_TIME_FORMAT, CultureInfo.InvariantCulture);
        }

        public static DateTime ParseDateTime(string dateTimeStr)
        {
            return DateTime.ParseExact(dateTimeStr, XSD_DATE_TIME_FORMAT, null);
        }
        ...
        public static DateTimeOffset ParseDateTimeOffset(string dateTimeOffsetStr)
        {
            ...
            if (dateTimeOffsetStr.StartsWith(ESCAPED_WCF_JSON_PREFIX, StringComparison.Ordinal) ||
                dateTimeOffsetStr.StartsWith(WCF_JSON_PREFIX, StringComparison.Ordinal))
            {
            ...
            {
                if (!dateTimeOffsetStr.EndsWith(XSD_UTC_SUFFIX)) dateTimeOffsetStr += XSD_UTC_SUFFIX;
                if (Env.IsMono)
                {
        ...
        public static string ToShortestXsdDateTimeString(DateTime dateTime)
        {
            ...
            if (isStartOfDay && !JsConfig.SkipDateTimeConversion)
                return dateTime.ToString(SHORT_DATE_TIME_FORMAT, CultureInfo.InvariantCulture);
            ...
            if (JsConfig.SkipDateTimeConversion)
            {
                if (!hasFractionalSecs)
                    return dateTime.Kind == DateTimeKind.Local
                        ? dateTime.ToString(DATE_TIME_FORMAT_SECONDS_UTC_OFFSET, CultureInfo.InvariantCulture)
                        : dateTime.Kind == DateTimeKind.Unspecified
                        ? dateTime.ToString(DATE_TIME_FORMAT_SECONDS_NO_OFFSET, CultureInfo.InvariantCulture)
                        : dateTime.ToStableUniversalTime().ToString(XSD_DATE_TIME_FORMAT_SECONDS, CultureInfo.InvariantCulture);

                return dateTime.Kind == DateTimeKind.Local
                    ? dateTime.ToString(DATE_TIME_FORMAT_TICKS_UTC_OFFSET, CultureInfo.InvariantCulture)
                    : dateTime.Kind == DateTimeKind.Unspecified
                    ? dateTime.ToString(DATE_TIME_FORMAT_TICKS_NO_UTC_OFFSET, CultureInfo.InvariantCulture)
        ...
        public static DateTimeOffset ParseWcfJsonDateOffset(string wcfJsonDate)
        {
            ...
            var suffixPos = wcfJsonDate.IndexOf(WCF_JSON_SUFFIX);
            var timeString = (suffixPos < 0) ? wcfJsonDate : wcfJsonDate.Substring(WCF_JSON_PREFIX.Length, suffixPos - WCF_JSON_PREFIX.Length);

            // for interop, do not assume format based on config
            if (!wcfJsonDate.StartsWith(WCF_JSON_PREFIX, StringComparison.Ordinal))
            {
            ...
            if (JsConfig.DateHandler == DateHandler.DcjsCompatible || timeZone == UNSPECIFIED_OFFSET)
            {
        ...
        {
            if (wcfJsonDate[0] == JsonUtils.ESCAPE_CHAR)
            {
                wcfJsonDate = wcfJsonDate.Substring(1);
            }

            var suffixPos = wcfJsonDate.IndexOf(WCF_JSON_SUFFIX);
            var timeString = wcfJsonDate.Substring(WCF_JSON_PREFIX.Length, suffixPos - WCF_JSON_PREFIX.Length);
        ...
        public static void WriteWcfJsonDate(TextWriter writer, DateTime dateTime)
        {
            ...
            writer.Write(ESCAPED_WCF_JSON_PREFIX);
            writer.Write(timestamp);
            if (offset != null)
            {
                writer.Write(offset);
            }
            writer.Write(ESCAPED_WCF_JSON_SUFFIX);
        }
