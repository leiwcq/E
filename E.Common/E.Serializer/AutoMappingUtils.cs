...
{
    [DataContract(Namespace = "http://schemas.e.net/types")]
    ...
    public static class AutoMappingUtils
    {
        ...
        public static TTo ThenDo<TTo>(this TTo to, Action<TTo> fn)
        {
            fn(to);
            return to;
        }
        ...
        {
            return E.Common.PclExport.Instance.GetAssemblyPath(source);
        }

        public static bool IsDebugBuild(this Assembly assembly)
        {
            return E.Common.PclExport.Instance.IsDebugBuild(assembly);
        }
        ...
        private static Dictionary<Type, object> _defaultValueTypes = new Dictionary<Type, object>();
        ...
        public static object GetDefaultValue(this Type type)
        {
            ...
            do
            {
                ...
                Interlocked.CompareExchange(ref _defaultValueTypes, newCache, snapshot), snapshot));
        ...
        public static TTo PopulateWith<TTo, TFrom>(this TTo to, TFrom from)
        {
            if (Equals(to, default(TTo)) || Equals(from, default(TFrom))) return default(TTo);
        ...
        public static TTo PopulateWithNonDefaultValues<TTo, TFrom>(this TTo to, TFrom from)
        {
            if (Equals(to, default(TTo)) || Equals(from, default(TFrom))) return default(TTo);
        ...
        public static TTo PopulateFromPropertiesWithAttribute<TTo, TFrom>(this TTo to, TFrom from,
            Type attributeType)
        {
            if (Equals(to, default(TTo)) || Equals(from, default(TFrom))) return default(TTo);
        ...
        public static TTo PopulateFromPropertiesWithoutAttribute<TTo, TFrom>(this TTo to, TFrom from,
            Type attributeType)
        {
            if (Equals(to, default(TTo)) || Equals(from, default(TFrom))) return default(TTo);
        ...
        public static void SetProperty(this PropertyInfo propertyInfo, object obj, object value)
        {
            ...
            var propertySetMetodInfo = propertyInfo.GetSetMethod(true);
        ...
        public static bool IsUnsettableValue(FieldInfo fieldInfo, PropertyInfo propertyInfo)
        {
            ...
            {
                return E.Common.PclExport.Instance.InSameAssembly(propertyInfo.DeclaringType, typeof(object));
            }
        ...
        private const int MAX_RECURSION_LEVEL_FOR_DEFAULT_VALUES = 2; // do not nest a single type more than this deep.
        ...
        public static object CreateDefaultValue(Type type, Dictionary<Type, int> recursionInfo)
        {
            ...
            if (recurseLevel > MAX_RECURSION_LEVEL_FOR_DEFAULT_VALUES) return null;
            ...
            try // use a try/finally block to make sure we decrease the recursion level for this type no matter which code path we take,
            {
                ...
                if (hasEmptyConstructor)
                {
                    ...
                    var genericCollectionType = E.Common.PclExport.Instance.GetGenericCollectionType(type);
