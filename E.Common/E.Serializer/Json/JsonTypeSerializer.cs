...
namespace E.Serializer.Json
{
        ...
        : ITypeSerializer
    {
        ...
        {
            if (JsState.WritingKeyCount > 0 && !JsState.IsWritingValue) writer.Write(JsonUtils.QUOTE_CHAR);
        ...
        internal static StringSegment ParseString(StringSegment json, ref int index)
        {
            ...
            if (buffer[offset + index] != JsonUtils.QUOTE_CHAR)
            ...
            do
            {
                ...
                if (c == JsonUtils.QUOTE_CHAR) break;
                if (c != JsonUtils.ESCAPE_CHAR) continue;
        ...
        public StringSegment UnescapeSafeString(StringSegment value)
        {
            ...
            return value.GetChar(0) == JsonUtils.QUOTE_CHAR && value.GetChar(value.Length - 1) == JsonUtils.QUOTE_CHAR
        ...
        static readonly char[] IsSafeJsonChars = new[] { JsonUtils.QUOTE_CHAR, JsonUtils.ESCAPE_CHAR };
        ...
        private static StringSegment UnEscapeJsonString(StringSegment json, ref int index)
        {
            ...
            if (firstChar == JsonUtils.QUOTE_CHAR)
            {
                index++;
                ...
                if (json.GetChar(strEndPos) == JsonUtils.QUOTE_CHAR)
                {
            ...
            else
            {
                ...
                {
                    var c = buffer[i];
                    if (c == JsonUtils.QUOTE_CHAR || c == JsonUtils.ESCAPE_CHAR)
                        break;
                    i++;
                }
        ...
        public static StringSegment Unescape(StringSegment input, bool removeQuotes)
        {
            ...
            for (; count < length;)
            {
                if (removeQuotes)
                {
                    if (input.GetChar(count) == JsonUtils.QUOTE_CHAR)
                    {
                        if (start != count)
                        {
                ...
                if (input.GetChar(count) == JsonUtils.ESCAPE_CHAR)
                {
                    if (start != count)
                    {
                        output.Append(input.Buffer, input.Offset + start, count - start);
            ...
            switch (valueChar)
            {
                ...
                case JsWriter.MapStartChar:
                    while (++i < valueLength)
                    {
                        ...
                        if (valueChar == JsonUtils.ESCAPE_CHAR)
                        {
                            i++;
                            continue;
                        }
