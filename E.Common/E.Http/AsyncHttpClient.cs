        ...
        private void ParseUrlInternal(string url)
        {
            ...
            try
            {
                ...
                split.Skip(1).ToList().ForEach(s =>
                {
                    ...
                    try
                    {
                        if (!ContainsKey(Uri.UnescapeDataString(s.Substring(0, idx))))
                        {
                            Add(Uri.UnescapeDataString(s.Substring(0, idx)),
                                Uri.UnescapeDataString(s.Substring(idx + 1)));
                        }
                        else
                        {
                            this[Uri.UnescapeDataString(s.Substring(0, idx))] =
                                Uri.UnescapeDataString(s.Substring(idx + 1));
                        }
                    }
                    catch
